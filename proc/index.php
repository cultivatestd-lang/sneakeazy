...
if ($page === 'home' && !$searchQuery) {
    ...
    try {
        ...
        if ($filter === 'sale') {
            ...
            // TOP PICKS: Advanced Personalization
            if (!$currentUser) {
            ...
            if ($hasRatings) {
                // A. COLLABORATIVE FILTERING APPROACH (User has explicit history)
                ...
                foreach ($userTopProductIds as $likedPid) {
                    ...
                    foreach ($neighbors as $nPid => $nScore) {
                        if (!isset($collabMap[$nPid])) $collabMap[$nPid] = 0;
                ...
                // 4. In-Memory Scoring Loop (Super Fast)
                foreach ($allProducts as $p) {
                    $score = 0;
                    $pid = $p['id'];
                    ...
                    // Collaborative Boost (Pre-calculated)
                ...
                // B. COLD START / IMPLICIT PREFERENCE (KNN-like similarity)
                // User hasn't rated, so relies on views (implicit) and general preferences (Category/Brand affinity)
                foreach ($allProducts as $p) {
            ...
            // --- ROULETTE / DYNAMIC SORTING LOGIC ---
            // Instead of pure sort, we group by score ranges and shuffle within ranges
            // This ensures "Freshness" every time user opens the page.
